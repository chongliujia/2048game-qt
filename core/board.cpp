...
void Board::moveHorizontally(int i, int j, Direction dir){
    bool tileCollision;
    if(board[i][j] != NULL){
        bool tileCollisionLastRound = false;
        int newj;

        if(dir == RIGHT) newj = j + 1;
        else newj = j - 1;
        ...
        if(!inbounds(i, newj)){
            if(dir == RIGHT) board[i][dimension - 1] = board[i][j];
            else board[i][0] = board[i][j];
        }
        else{
            if(board[i][newj]->getValue() == board[i][j]->getValue() &&
               !board[i][newj]->getUpgratedThisMove()){
                ...
                if(dir == RIGHT) board[i][newj - 1] = board[i][j];
                else board[i][newj + 1] = board[i][j];
            }
        }
...
void Board::moveVertically(int i, int j, Direction dir){
    if(board[i][j] != NULL){
        ...
        if(!inbounds(newi, j)){
            if(dir == UP) board[0][j] = board[i][j];
            else board[dimension - 1][j] = board[i][j];
        }
        else{
            if(board[newi][j]->getValue() == board[i][j]->getValue() &&
               !board[newi][j]->getUpgratedThisMove()){
                ...
                if(dir == UP) board[newi + 1][j] = board[i][j];
                else board[newi - 1][j] = board[i][j];
            }
        }
...
void Board::handleCollision(int i, int j){
    ...
    pointScoredLastRound += board[i][j]->getValue();
}
